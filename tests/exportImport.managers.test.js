...
describe('profile backup format', () => {
  ...
  test('parseProfileBackup accepts the new format and normalizes', () => {
    ...
    expect(normalized.profile.userId).toBe('user-abc')
    expect(normalized.profile.displayName).toBe('User ABC')
    expect(Array.isArray(normalized.profile.prerequisites)).toBe(true)
    expect(normalized.profile.features.allowManualUnlock).toBe(true)
    expect(normalized.profile.features.enforceCurrentYearForSustained).toBe(false)
    expect(normalized.profile.notifications).toBe(true)
    expect(normalized.profile.prerequisites[0].id).toBeTruthy()
    expect(normalized.profile.sex).toBe('female')
  })
})
