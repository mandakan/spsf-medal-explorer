...
import { getRunningShootingCourseDefaults, getRequirementHint } from '../../utils/requirementDefaults'
import CollapsibleOptionalFields from '../CollapsibleOptionalFields'

/**
 * Type-specific form for logging running/skiing shooting course achievements.
...
export default function RunningShootingCourseForm({ medal, onSubmit, onSubmitAndAddAnother, loading, preservedValues }) {
  ...
  const defaults = useMemo(() => {
    return getRunningShootingCourseDefaults(medal, currentProfile)
  }, [medal, currentProfile])

  // Get contextual hint from medal requirements (try both running and skiing types)
  const requirementHint = useMemo(() => {
    return getRequirementHint(medal, defaults.achievementType) ||
           getRequirementHint(medal, 'running_shooting_course') ||
           getRequirementHint(medal, 'skis_shooting_course')
  }, [medal, defaults.achievementType])
  ...
  const { values, errors, handleChange, handleSubmit, validate, setErrors } = useAchievementForm({
    ...
    validate: (vals) => {
      ...
      if (vals.points === '' || vals.points === undefined) {
        ...
        errs.points = 'Poäng måste vara 0 eller högre'
      }

      return errs
    },
    onSubmit: (vals) => onSubmit({ ...vals, achievementType: defaults.achievementType || 'running_shooting_course' }),
  })
  ...
  const handleSaveAndAddAnother = (e) => {
    ...
    if (Object.keys(validationErrors).length === 0 && onSubmitAndAddAnother) {
      onSubmitAndAddAnother({ ...values, achievementType: defaults.achievementType || 'running_shooting_course' })
    }
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-4" noValidate>
      ...
      <div>
        ...
        <input
          ...
          type="date"
          name="date"
          aria-label="Datum för terränglopp"
      ...
      <CollapsibleOptionalFields label="Valfria fält">
        ...
        <div>
          ...
          <input
            ...
            type="text"
            name="courseName"
            aria-label="Bana eller tävlingsnamn"
            value={values.courseName}
            onChange={handleChange}
            className="input py-3"
            placeholder="T.ex. Klubbmästerskap, DM Springskytte"
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            placeholder="T.ex. väderförhållanden, utrustning, etc."
          />
        </div>
